# Vagrantfile
# Version pinning to ensure consistent Vagrant behavior:
...
Vagrant.configure("2") do |config|
  ...
  # New improvement: add cluster version info banner
  config.vm.provision "shell", inline: <<-SHELL
    echo "====================================="
    echo " Starting Kubernetes v#{settings['software']['kubernetes']} cluster"
    echo " Master: #{settings['network']['control_ip']}"
  ...
  # Box selection with arch awareness:
